...
{
    /// <summary>
    /// Copy of AesCryptoProvider class from the updated repository of official Github.
    /// </summary>
    [HostProtection(SecurityAction.LinkDemand, MayLeakOnAbort = true)]
    public sealed class AesCryptoProviderCustom : Aes
    {
        public AesCryptoProviderCustom()
        {
            FeedbackSizeValue = 8;
        }
        ...
        public override ICryptoTransform CreateDecryptor(byte[] key, byte[] iv)
        {
            ...
            return (ICryptoTransform)new AesTransform((Aes)this, false, key, iv);
        }
        ...
        public override ICryptoTransform CreateEncryptor(byte[] key, byte[] iv)
        {
            if (Mode == CipherMode.CFB && FeedbackSize > 64)
                throw new CryptographicException("CFB with Feedbaack > 64 bits");
            return (ICryptoTransform)new AesTransform((Aes)this, true, key, iv);
        }
    ...
    internal class AesTransform : SymmetricTransform
    {
        ...
        public AesTransform(Aes algo, bool encryption, byte[] key, byte[] iv) : base(algo, encryption, iv)
        {
            ...
            {
                string msg = "IV length is invalid ({0} bytes), it should be {1} bytes long.";
            ...
            {
                string msg = "Key is too small ({0} bytes), it should be {1}, {2} or {3} bytes long.";
                throw new CryptographicException(msg);
            }
    ...
    internal abstract class SymmetricTransform : ICryptoTransform, IDisposable
    {
        ...
        public SymmetricTransform(SymmetricAlgorithm symmAlgo, bool encryption, byte[] rgbIV)
        {
            ...
            Buffer.BlockCopy((Array)rgbIV, 0, (Array)temp, 0, Math.Min(BlockSizeByte, rgbIV.Length));
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            if (disposing)
            {
                Array.Clear((Array)temp, 0, BlockSizeByte);
                temp = (byte[])null;
                Array.Clear((Array)temp2, 0, BlockSizeByte);
                temp2 = (byte[])null;
            }
            m_disposed = true;
        }
        ...
        protected virtual void CBC(byte[] input, byte[] output)
        {
            if (encrypt)
            {
                for (int index = 0; index < BlockSizeByte; ++index)
                    temp[index] ^= input[index];
                ECB(temp, output);
                Buffer.BlockCopy((Array)output, 0, (Array)temp, 0, BlockSizeByte);
            }
            else
            {
                Buffer.BlockCopy((Array)input, 0, (Array)temp2, 0, BlockSizeByte);
                ECB(input, output);
                for (int index = 0; index < BlockSizeByte; ++index)
                    output[index] ^= temp[index];
                Buffer.BlockCopy((Array)temp2, 0, (Array)temp, 0, BlockSizeByte);
            }
        }
        ...
        protected virtual void CFB(byte[] input, byte[] output)
        {
            if (encrypt)
            {
                for (int srcOffset = 0; srcOffset < BlockSizeByte; ++srcOffset)
                {
                    ECB(temp, temp2);
                    output[srcOffset] = (byte)((uint)temp2[0] ^ (uint)input[srcOffset]);
                    Buffer.BlockCopy((Array)temp, 1, (Array)temp, 0, BlockSizeByte - 1);
                    Buffer.BlockCopy((Array)output, srcOffset, (Array)temp, BlockSizeByte - 1, 1);
                }
            }
            else
            {
                for (int srcOffset = 0; srcOffset < BlockSizeByte; ++srcOffset)
                {
                    ...
                    encrypt = false;
                    Buffer.BlockCopy((Array)temp, 1, (Array)temp, 0, BlockSizeByte - 1);
                    Buffer.BlockCopy((Array)input, srcOffset, (Array)temp, BlockSizeByte - 1, 1);
                    output[srcOffset] = (byte)((uint)temp2[0] ^ (uint)input[srcOffset]);
                }
            }
        }
        ...
        private void CheckInput(byte[] inputBuffer, int inputOffset, int inputCount)
        {
            ...
            if (inputOffset > inputBuffer.Length - inputCount)
                throw new ArgumentException(nameof(inputBuffer));
        }
        ...
        public virtual int TransformBlock(byte[] inputBuffer, int inputOffset, int inputCount, byte[] outputBuffer, int outputOffset)
        {
            ...
            if (!encrypt && 0 > num && (padmode == PaddingMode.None || padmode == PaddingMode.Zeros))
                throw new CryptographicException(nameof(outputBuffer));
            if (KeepLastBlock)
            {
        ...
        private int InternalTransformBlock(byte[] inputBuffer, int inputOffset, int inputCount, byte[] outputBuffer, int outputOffset)
        {
            ...
            if (lastBlock)
            {
                ...
                Buffer.BlockCopy((Array)workout, 0, (Array)outputBuffer, outputOffset, BlockSizeByte);
            ...
            {
                Buffer.BlockCopy((Array)inputBuffer, srcOffset, (Array)workBuff, 0, BlockSizeByte);
                Transform(workBuff, workout);
                Buffer.BlockCopy((Array)workout, 0, (Array)outputBuffer, outputOffset, BlockSizeByte);
                srcOffset += BlockSizeByte;
                outputOffset += BlockSizeByte;
                num2 += BlockSizeByte;
            }
            if (KeepLastBlock)
        ...
        {
            string message = string.Format("Bad {0} padding.", (object)padding);
            if (length >= 0)
                message += string.Format(" Invalid length {0}.", (object)length);
            if (position >= 0)
                message += string.Format(" Error found at position {0}.", (object)position);
            throw new CryptographicException(message);
        }

        protected virtual byte[] FinalEncrypt(byte[] inputBuffer, int inputOffset, int inputCount)
        {
            int num1 = inputCount / BlockSizeByte * BlockSizeByte;
            int count = inputCount - num1;
            int length1 = num1;
            PaddingMode padmode = this.padmode;
            if (padmode == PaddingMode.PKCS7 || (uint)(padmode - 4) <= 1U)
            {
                length1 += BlockSizeByte;
            }
            else
            {
                if (inputCount == 0)
                ...
                if (count != 0)
                {
                    ...
                    byte[] numArray = new byte[num1 + BlockSizeByte];
            ...
            int outputOffset = 0;
            while (length1 > BlockSizeByte)
            {
                InternalTransformBlock(inputBuffer, inputOffset, BlockSizeByte, numArray1, outputOffset);
                inputOffset += BlockSizeByte;
                outputOffset += BlockSizeByte;
                length1 -= BlockSizeByte;
            }
            byte num2 = (byte)(BlockSizeByte - count);
            switch (padmode)
            {
                case PaddingMode.PKCS7:
                    ...
                    InternalTransformBlock(numArray1, num1, BlockSizeByte, numArray1, num1);
                    break;
                ...
                case PaddingMode.ISO10126:
                    Random(numArray1, numArray1.Length - (int)num2, (int)num2 - 1);
                    numArray1[numArray1.Length - 1] = num2;
                    Buffer.BlockCopy((Array)inputBuffer, inputOffset, (Array)numArray1, num1, count);
                    InternalTransformBlock(numArray1, num1, BlockSizeByte, numArray1, num1);
                    break;
        ...
        protected virtual byte[] FinalDecrypt(byte[] inputBuffer, int inputOffset, int inputCount)
        {
            ...
            while (num1 > 0)
            {
                int num3 = InternalTransformBlock(inputBuffer, inputOffset, BlockSizeByte, outputBuffer, num2);
                inputOffset += BlockSizeByte;
                num2 += num3;
                num1 -= BlockSizeByte;
            }
            ...
            {
                Transform(workBuff, workout);
                Buffer.BlockCopy((Array)workout, 0, (Array)outputBuffer, num2, BlockSizeByte);
                int num3 = num2 + BlockSizeByte;
                lastBlock = false;
            }
            byte num4 = count > 0 ? outputBuffer[count - 1] : (byte)0;
            switch (padmode)
            {
                case PaddingMode.PKCS7:
                    if (num4 == (byte)0 || (int)num4 > BlockSizeByte)
                        ThrowBadPaddingException(padmode, (int)num4, -1);
                    for (int position = (int)num4 - 1; position > 0; --position)
                    {
                        if ((int)outputBuffer[count - 1 - position] != (int)num4)
                            ThrowBadPaddingException(padmode, -1, position);
                    }
                    count -= (int)num4;
                    break;
                case PaddingMode.ANSIX923:
